    ...
    SampleUniquePtr<nvcaffeparser1::ICaffeParser>& parser, DataType dataType)
{
    ...
    config->setMaxWorkspaceSize(1000000000);
    ...
    std::string trt_file_name = "default.trt";
    if (dataType == DataType::kFLOAT) {
      trt_file_name = "kFLOAT.trt";
    } else if (dataType == DataType::kHALF) {
      trt_file_name = "kHALF.trt";
    } else if (dataType == DataType::kINT8) {
      trt_file_name = "kINT8.trt";
    } else {
      std::cout << "=============> [error] Unknown data type" << std::endl;
    }
    saveEngine(*mEngine, trt_file_name, std::cout);
...
bool SampleINT8::infer(std::vector<float>& score, int firstScoreBatch, int nbScoreBatches)
{
    ...
    std::cout << "=====================> outputTensorNames: "
              << mParams.outputTensorNames[0] << ", output size: " << outputSize
              << std::endl;
    int top1{0}, top5{0};
